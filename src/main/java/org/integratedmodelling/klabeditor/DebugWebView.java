...
public class DebugWebView extends StackPane {
  ...
  private void setupDebugRedirection(WebEngine engine) {
    // Intercept any attempt to load content; if there is a concrete location (URL), open it
    engine.locationProperty().addListener(
        (obs, oldLocation, newLocation) -> {
          if (newLocation != null && isLikelyUrl(newLocation)) {
            cancelIfRunning(engine);
            openExternal(newLocation);
          }
        });

    // If a load is scheduled or running without an explicit URL (e.g., loadContent), cancel it.
    engine
        .getLoadWorker()
        .stateProperty()
        .addListener(
            (obs, oldState, newState) -> {
              if (newState == Worker.State.SCHEDULED || newState == Worker.State.RUNNING) {
                // If there is a URL we will open it via the location listener above; otherwise just cancel
                cancelIfRunning(engine);
              }
            });
  ...
  private static void cancelIfRunning(WebEngine engine) {
    var worker = engine.getLoadWorker();
    if (worker.getState() == Worker.State.SCHEDULED || worker.getState() == Worker.State.RUNNING) {
      // Cancel on FX thread
